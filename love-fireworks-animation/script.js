...
function render(speed) {
    ...
    COLOR_CODES.forEach(color => {
        ...
        ctx.fill();//填充绘制的心形路径。
        ctx.closePath();//关闭当前绘图路径。
        ctx.restore();//恢复绘图状态：恢复到之前保存的绘图状态，撤销之前的平移和旋转操作。
        // 创建图像对象
        const particleImage = new Image();
        particleImage.src = canvas.toDataURL(); // 将临时画布转换为图像数据
        ...
        stars.forEach(star => {
			if (star.visible) {
				...
				image.sizes= '1'
				// 在每个拖尾位置绘制图像
				trailsCtx.drawImage(particleImage,star.x-20, star.y-15,40,30);
	...
	COLOR_CODES.forEach(color => {
		...
		//trailsCtx.stroke() 是一个用于在画布上绘制路径轮廓的方法。它会根据当前设置的样式和属性，沿着路径的轮廓绘制线条。
		trailsCtx.stroke();
	});
